  ...
  return (
    <View className="mt-12 mb-16 bg-secondary rounded-[32px] p-8 border border-gray-100 flex-row items-center">
      <View className={`flex-1 ${isRTL ? 'items-end' : 'items-start'}`}>
        ...
        <Text className={`text-gray-500 font-[Cairo_400Regular] leading-6 mb-4 ${isRTL ? 'text-right' : 'text-left'}`}>
          {bio || (isRTL
        ...
        <View className="flex-row">
          <TouchableOpacity className="bg-primary/5 p-2 rounded-full mr-3">
      ...
      <Image
        ...
        className={`w-28 h-28 rounded-3xl ${isRTL ? 'ml-6' : 'mr-6'}`}
      />
    </View>
  );
};
